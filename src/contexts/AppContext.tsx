...
import { gruposItens as initialGrupos, concessionarias, orcamentos as initialOrcamentos, mockDB } from '../data/mockData';
import { supabase } from '../lib/supabaseClient';
import { processAndUploadMaterials } from '../services/materialImportService';

// MODO MOCKADO ATIVADO - Todas as operações usam dados simulados
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const fetchMaterials = useCallback(async () => {
    try {
      ...
      if (MOCK_MODE) {
        // Modo mockado: busca do banco de dados simulado
        const materials = mockDB.getMaterials();
        setMateriais(materials);
      } else {
        // Modo real: busca do Supabase (mantido para referência)
  ...
  const addMaterial = async (material: Omit<Material, 'id'>) => {
    try {
      if (MOCK_MODE) {
        // Modo mockado: adiciona no banco simulado
        ...
        // Modo real: adiciona no Supabase (mantido para referência)
        const materialData = {
  ...
  const updateMaterial = async (id: string, material: Omit<Material, 'id'>) => {
    try {
      if (MOCK_MODE) {
        // Modo mockado: atualiza no banco simulado
        ...
        console.log("💰 Material mockado atualizado");
      } else {
        // Modo real: atualiza no Supabase (mantido para referência)
        const materialData = {
          code: material.codigo,
          name: material.descricao,
          price: material.precoUnit,
  ...
  const deleteMaterial = async (id: string) => {
    try {
      if (MOCK_MODE) {
        // Modo mockado: remove do banco simulado
        mockDB.deleteMaterial(id);
        setMateriais(prev => prev.filter(m => m.id !== id));
      } else {
        // Modo real: remove do Supabase (mantido para referência)
  ...
  const deleteAllMaterials = async () => {
    try {
      if (MOCK_MODE) {
        // Modo mockado: limpa todos os materiais do banco simulado
  ...
  const fetchBudgets = useCallback(async () => {
    try {
      ...
      if (MOCK_MODE) {
        // Modo mockado: busca do banco simulado
      ...
      const data = await fetchAllRecords('budgets', selectQuery, 'created_at', false);
  ...
  const updateBudget = async (budgetId: string, budgetData: { project_name?: string; client_name?: string; city?: string; company_id?: string; }) => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .update(updateData)
        .eq('id', budgetId)
 // Garantir que só pode editar seus próprios orçamentos
        .select()
        .single();
  ...
  const deleteBudget = async (budgetId: string) => {
    try {
      const { error } = await supabase
        ...
        .delete()
        .eq('id', budgetId)
; // Garantir que só pode excluir seus próprios orçamentos
    ...
    try {
      ...
      const { data: newMaterial, error: materialError } = await supabase
        ...
        .insert({
          ...
          unit: 'unidade',
          price: data.price,
 // Adicionar user_id para isolamento de dados
        })
        .select()
        .single();
