...
import { Plus, Calendar, Building2, Loader2, Edit, Trash2, Copy, CheckCircle, Clock, BarChart3, TrendingUp, Search, Filter, X, Folder, FolderOpen, MoreVertical, FolderEdit, FileText, Calculator, Home } from 'lucide-react';
...
export function Dashboard() {
  ...
  const handlePrecificar = (budget: Orcamento) => {
    setCurrentOrcamento(budget);
    setCurrentView('precificacao');
  };
          ...
          {budget.status !== 'Finalizado' ? (
            ...
            <>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  handlePrecificar(budget);
                }}
                className="px-3 py-1.5 text-xs font-medium bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded hover:from-blue-700 hover:to-purple-700 transition-all flex items-center gap-1.5 shadow-sm"
                title="Precificação"
              >
